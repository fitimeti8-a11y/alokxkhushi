...
async def send_and_wait(username: str) -> dict:
    ...
    msg_to_send = f"t.me/{username}"

    try:
        sent = await tg_client.send_message(TARGET_BOT, msg_to_send)
    ...
    return {"success": False, "error": "No reply from bot after 60 seconds"}
...
def check():
    global tg_ready
    if not tg_ready:
        return jsonify({"success": False, "error": "Telegram not ready yet"})
    ...
    try:
        fut = asyncio.run_coroutine_threadsafe(send_and_wait(username), tg_loop)
